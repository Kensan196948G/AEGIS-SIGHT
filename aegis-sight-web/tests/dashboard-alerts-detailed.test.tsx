...
function renderAlerts() {
  ...
  const Page = require('@/app/dashboard/alerts/page').default;
  return render(<Page />);
}
...
describe('Alerts page - basic rendering', () => {
  ...
  it('displays page heading "アラート管理"', () => {
    renderAlerts();
  ...
  it('renders all 7 demo alerts by default', () => {
    renderAlerts();
  ...
  it('renders three filter selects', () => {
    renderAlerts();
    const selects = document.querySelectorAll('select');
...
describe('Alerts page - stats cards', () => {
  it('displays total count 47', () => {
    renderAlerts();
    expect(screen.getByText('47')).toBeTruthy();
    expect(screen.getByText('合計')).toBeTruthy();
  });

  it('displays critical count 8', () => {
    renderAlerts();
    expect(screen.getByText('8')).toBeTruthy();
    expect(screen.getByText('重大')).toBeTruthy();
  });

  it('displays warning count 21', () => {
    renderAlerts();
    expect(screen.getByText('21')).toBeTruthy();
  });

  it('displays info count 18', () => {
    renderAlerts();
    expect(screen.getByText('18')).toBeTruthy();
  });

  it('displays unacknowledged count 15', () => {
    renderAlerts();
    expect(screen.getByText('15')).toBeTruthy();
    expect(screen.getByText('未確認')).toBeTruthy();
  });

  it('displays unresolved count 32', () => {
    renderAlerts();
...
describe('Alerts page - resolve rate chart', () => {
  it('renders DonutChart with resolve rate label', () => {
    renderAlerts();
  ...
  it('uses red color (#ef4444) when resolveRate < 60', () => {
    renderAlerts();
    const donut = screen.getByTestId('donut-chart');
    // 32% < 60 → red
  ...
  it('renders BarChart with severity labels', () => {
    renderAlerts();
...
describe('Alerts page - getAlertStatus branches', () => {
  it('shows "未対応" for alerts with !resolved_at && !is_acknowledged (open)', () => {
    renderAlerts();
  ...
  it('shows "確認済" for alerts with is_acknowledged && !resolved_at (acknowledged)', () => {
    renderAlerts();
  ...
  it('shows "解決済" for alerts with resolved_at truthy (resolved)', () => {
    renderAlerts();
...
describe('Alerts page - status badge variants', () => {
  it('uses "danger" variant for open status', () => {
    renderAlerts();
  ...
  it('uses "info" variant for acknowledged status', () => {
    renderAlerts();
  ...
  it('uses "success" variant for resolved status', () => {
    renderAlerts();
...
describe('Alerts page - severity badge variants', () => {
  it('uses "danger" variant with dot for critical alerts', () => {
    renderAlerts();
  ...
  it('uses "warning" variant with dot for warning alerts', () => {
    renderAlerts();
  ...
  it('uses "info" variant with dot for info alerts', () => {
    renderAlerts();
...
describe('Alerts page - conditional buttons', () => {
  it('renders 確認 button only for unacknowledged alerts', () => {
    renderAlerts();
  ...
  it('renders 解決 button only for unresolved alerts', () => {
    renderAlerts();
  ...
  it('does NOT render 確認 button for acknowledged alerts', () => {
    renderAlerts();
  ...
  it('does NOT render 解決 button for resolved alerts', () => {
    renderAlerts();
...
describe('Alerts page - severity filter', () => {
  ...
  it('resets to all when "all" selected', () => {
    renderAlerts();
    const { severity } = getSelects();
    fireEvent.change(severity, { target: { value: 'critical' } });
...
describe('Alerts page - status filter', () => {
  ...
  it('filters to acknowledged only', () => {
    renderAlerts();
    const { status } = getSelects();
...
describe('Alerts page - combined filters', () => {
  it('combines severity + category filter', () => {
    renderAlerts();
  ...
  it('empty state row has colSpan 6', () => {
    renderAlerts();
    const { severity, category } = getSelects();
    fireEvent.change(severity, { target: { value: 'critical' } });
    fireEvent.change(category, { target: { value: 'network' } });
...
describe('Alerts page - alert content', () => {
  ...
  it('displays category labels in table', () => {
    renderAlerts();
