...
**§1.5 残日数別の自動縮退ポリシー（絶対厳守）:**

| 残日数 | フェーズ呼称 | 許容される作業 | 禁止される作業 |
|---|---|---|---|
| 60 日以内（残2ヶ月） | **Stabilize Phase** | バグ修正・安定化・テスト追加・docs整備・soft-gate の hard 化 | 大型新機能・破壊的依存更新（major bump） |
| 30 日以内（残1ヶ月） | **Release Prep Phase** | バグ修正・移行手順整備・運用引継ぎ・ロールバック手順整備・UAT 残課題消化 | Improvement の自由活用、未検証 PR、新機能（緊急 fix を除く） |
| 14 日以内（残2週間） | **Code Freeze Phase** | バグ修正のみ・CHANGELOG 整備・タグ準備 | 新機能開発、リファクタ、依存更新 |
| 7 日以内（残1週間） | **Release Window** | リリース準備のみ（CHANGELOG 確定・README 確定・タグ付け・本番デプロイ・監査証跡確認） | 機能追加、構造変更（Critical/Security 例外のみ） |

> 🛡️ **Stabilize/Code Freeze 期に新機能 PR が来たら**: 自動的に Backlog（次バージョン）へ送る。例外は Critical/Security/データ破損リスクのみ。
> 📅 **本番判定の 3 段階**: ① Hard CI Gate 完了 → ② UAT / 実機 / セキュリティ検証完了 → ③ 運用引継ぎ / ロールバック / 監査証跡確認完了。3 つそろうまで「機能完成」≠「本番Ready」。
...
## 1.5. プロジェクト期間・リリース制約（最優先・絶対厳守）

本プロジェクトは **6 ヶ月期間** で運用する。以下は他のすべての方針に優先する最上位制約である。

| 項目 | 値 |
|---|---|
| プロジェクト期間 | **6 ヶ月（登録日から半年）** — 登録日: **2026-03-25** / 期限: **2026-09-25** |
| 実行方式 | **Linux Cron** によるプロジェクト別自動スケジュール（月〜土） |
| 1 セッション最大作業時間 | **5 時間（300 分）厳守** |
| 開発フェーズ配分 | 進捗に応じて **CTO 判断で自由に変更可**（Monitor / Build / Verify / Improve の比率は動的） |
| **本番リリース期限** | **登録から 6 ヶ月後（絶対厳守）** — 2026-09-25 は動かさない |

### 残日数による自動縮退ルール（4 階層 + 3 段階本番判定）

ステップ 3 の §1.5 表（残 60 / 30 / 14 / 7 日）を正本とする。本セクションは要約と運用補足。

### 期間内の 6 ヶ月分割計画

| 月 | フェーズ |
|---|---|
| Month 1〜2 | 基盤整備・主要機能実装 |
| Month 3〜4 | 品質向上・テスト整備 |
| Month 5 | 統合テスト・バグ修正 |
| Month 6 | リリース準備・本番移行 |

### Monitor フェーズで必須の残日数確認

- 毎セッション Monitor フェーズで `state.json.project.release_deadline` と `project.registered_at` を参照し、残日数を記録すること
- 残日数と現在フェーズの整合性をチェックし、縮退ポリシーに違反していれば CTO が即座に方針を修正する
- GitHub milestone（v1.0 production release, due **2026-09-25**）と本ファイルの期限は常に一致させる
